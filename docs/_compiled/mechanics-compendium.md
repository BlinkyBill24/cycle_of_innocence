...
- **Bell pattern language** ✅ **built 2026-06-13** (volume-tuned same day after playtest "barely noticeable"): `_world_lurches()` tolls the chapel bell **once per stage reached**, spaced ~0.85 s (`bell_pattern(i)`, pure/tested), at **+6 dB and ducking the adaptive music** under the peal so the tolls cut through — attentive players can literally *count* the doom from the sound alone. No UI.
