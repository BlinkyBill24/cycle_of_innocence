...
- Generic soothe plateaus at Recognition 60; each monster exports `soothe_key_flag` — Briar digging up the keyed memory IS the discovery payoff. With the key: rate ×1.6 and the plateau lifts. **The key must be diggable in the monster's OWN zone** (playtest 2026-06-21: the fringes monsters defaulted to the *playground* rabbit's `dug_playground_buried_toy`, unreachable in-zone, so the soothe always stalled). The fringes TwistedChildren now key off the in-zone shoe dig (`dug_fringes_buried_memory`). The glance cue points at `soothe_key_spot_path` (the key dig) which can differ from `secret_spot_path` (where a Stilled child later *leads*).
