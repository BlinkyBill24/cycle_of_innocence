...
- **De-risk Web audio in an EXPORTED build before scoping the slice further** (solo-dev setup research, 2026-06-20): browsers block all audio until the first click/tap, and Web audio latency/crossfade timing is looser than in-editor — so "it works on F5" does NOT prove it works on Web. Export the current build to the Web target and confirm the adaptive stems (a) start only after a user interaction and (b) crossfade acceptably. This is load-bearing for *horror*: if the stems don't play on Web, the dread doesn't land. Distinct from the Sample-mode note above (that's no-DSP; this is autoplay-block + timing). LimboAI-on-Web is already proven; audio is the open Web risk. `[verified 2026-06-20]`
