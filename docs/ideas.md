...
- **Content Studio Loop** (from [[research/done/2026-07-24-one-person-ai-game-studio-claims-and-sorceress]]): for each small bite — brief → code against existing systems → generate only missing assets → godot-mcp smoke + GUT → human dread/bond gate. Process only; not a new engine. Promote to short design note when we run Phase 1.
- **Agentic play-path smoke (Phase 1)** — document + implement recipe: run_project → hollow-house key path → inventory → one dread zone → screenshots + no SCRIPT ERROR + `tools/run-tests.sh`. Branch name when built: `feature/agentic-playtest-smoke`.
...
- **Flute-gate monster interaction.** ✅ **SOOTHE half DONE 2026-06-21**: the `flute` item (`flute.tres`, grants the `flute_found` flag via new `ItemDef.grants_flag` on acquisition) gates ALL soothing — `player_controller._soothe_unlocked()` silently blocks the soothe entry point + prompt pre-flute; playing it reuses the existing soothe + a diegetic `flute` SFX. Persisted; debug-grant on **F**. `test_flute_gate.gd`. ⏳ **STILL TODO**: the COMBAT half — weapons should also do nothing to monsters pre-flute (pairs with the bare-fists-no-damage task; the hitbox/faction path would gate on `flute_found`). The allied-glow keys off `stilled` (only reachable post-flute), so it's already consistent.
...
## 📥 Captured this session (Godot MCP tooling, 2026-06-28)

- ~~**Add a runtime-visibility Godot MCP server**~~ **DONE 2026-06-28** — adopted **`Erodenn/godot-mcp-runtime` v3.1.2** (`npx -y godot-mcp-runtime@3.1.2` in `.mcp.json`, env `GODOT_PATH`), replacing the old `Coding-Solo/godot-mcp` base. Chosen for the hard Web constraint: addon-free, injects its bridge at runtime → **nothing ships in web/Android export, no export-exclude needed**, and it sidesteps Coding-Solo's command-injection advisory. Boot-smoke validated the run+capture loop first (game boots clean, only warnings). Now exposes the live runtime tree (`get_scene_tree`/`get_node_properties`), autoload state (`list_autoloads` → DreadManager/HollowingClock), `run_script` in the running game, screenshots, input sim. **Loads only after a Claude Code restart** (project-scoped MCP approval). Unlocks the next jobs below. `Godot-Peek-MCP` stays the fallback if stack-traces/perf-monitors are ever needed (but ships an addon + Linux x86_64-only); `IvanMurzak/Godot-MCP` stays disqualified (C#/.NET). See [[research/done/Claude-Godot-MCP-Servers-Research-2026-06-28]] + [[design/ai-production-setup]].
  - ~~**NEXT (needs the runtime server live)**~~ **DIAGNOSED 2026-06-28** (live-runtime pass, see [[sessions/2026-06-28-godot-runtime-diagnostics]]): (1) **Briar "yellow blob" = legibility, NOT wiring.** Drove `command_seek` live — it returns true, the state machine runs follow→seek→LEAD→TELL→follow and spawns the exclaim every time. The tell is `assets/sprites/ui/exclaim.png`, a **6×8 px pale-yellow placeholder "!"** drawn at 12×16 px — a tiny illegible speck. **Fix is art** (a real, larger, higher-contrast point-tell), not code. (2) **Locked door "no feedback" = silent + non-reactive, NOT wiring.** `trigger()` correctly returns false and shows a *legible* white floating label, but there's **no sound** on a locked interact and the label is already up from `body_entered`, so the press changes nothing on screen. **Fix = a reactive cue** (a "locked" thunk — `stinger_toy` could stand in — + a label nudge/flash); it's game-*feel*, so human-call. (3) **Autoloads healthy** — DreadManager tier 0 (calm), HollowingClock/VillageState/PlayerData loaded; `Inventory` is a static `class_name` class, not a node autoload (expected). ~~Both fixes left for human sign-off~~ **FIXED 2026-06-28** (see [[sessions/2026-06-28-tell-and-door-feedback-fixes]]): (1) bold dark-outlined 10×14 "!" replaces the pale-blob tell (`gen_ui_icons.py` + reimport, spawn nudged to y=-24); (2) locked doors now play a "locked" thunk (`stinger_toy` stand-in) + shake/flash the reason label on a blocked interact (`door_transition._reject_feedback`, debounced). 388/388 tests green, both verified live. ~~Remaining backlog: a bespoke "locked" SFX~~ **DONE 2026-06-28** — `door_locked.wav` (ElevenLabs rattle + bolt thunk, peak-normalized) now plays instead of the toy-creak stand-in (see [[sessions/2026-06-28-locked-door-sfx]]).
  - **Cleanup tail:** the old `Coding-Solo/godot-mcp` install still sits at `~/godot-mcp` (unused, off the repo) — can be deleted whenever. First boot-smoke also surfaced a code-hygiene baseline worth a pass: 2 stale UID warnings in `playground_fringes.tscn` (campfire/fog `.tres` — fix via `update_project_uids`), an *integer-division* warning, two `village_state.gd` static methods called from an instance, ~4 ternary type-mismatch warnings, and several `name`/`age` shadowing/unused-var warnings. None fatal (game boots); fold into a future chore.
...
- ✅ DONE 2026-06-20 — Pick-up-throw verb: `ThrowableObject` (Web-safe FAKED arc — a parabola on the Visual's y + ground shadow, NO RigidBody2D). Interact lifts a loose object (it hovers above Rowan), interact/attack hurls it in the facing dir; it arcs, lands, rests (or breaks). Airborne it carries a **player-faction `Hitbox`**, so hitting a monster routes through the SAME `Hitbox`/`Faction` path as the slingshot (one combat system). `GameEvents.throwable_picked_up/thrown/landed`. One placed in the playground (`playground_rock`). `test_throwable.gd`.
- ✅ DONE 2026-06-21 — Throw-at-NPC reaction (ONE showcase NPC, **Marta**): a thrown object (shared `thrown` group) striking Marta fires a **scripted, state-keyed** reaction — an authored line (`marta_throw_reaction.dialogue`: calm/wary/afraid by HollowingClock stage + suspicion) floated as a diegetic balloon + a flinch; raises VillageState suspicion (persisted) + nudges morality; `GameEvents.npc_reacted_to_throw`. `NpcThrowReaction` component on Marta only. `test_npc_throw_reaction.gd`. **GUARDRAIL HELD**: fixed table, current-state only — NO learning/memory/grudge/pursuit.
- ✅ DONE 2026-06-21 — Throw-reaction SCALED to the village (probe validated — Marta read well in playtest). `NpcThrowReaction.reaction_dialogue_path` is now per-NPC (each points at its own authored .dialogue, so the reaction is specific to WHO they are). Added reactions for **Pieter** (a father who can't place your face), **Elder Aldwin** (recognition, not fear), **Warden Brek** (authority→threat), **Lena** (a child's terror), and **Warden Oslo** (the searcher realizing the ritual didn't take). Same guardrail held — scripted, current-state only, no learning/pursuit. `test_npc_throw_reaction.gd`. Lines are DRAFT bible content.
- 🔬 VALIDATE-BEFORE-SCALE (Marta reaction): this is a PROBE. Playtest question — do players read the reaction as meaningful (the conspiracy showing through how she reacts)? Only AFTER it reads, author more reactive NPCs. Do NOT roll out village-wide blindly. The reaction lines are DRAFT (bible lore) — refine Marta's calm/wary/afraid lines. No dedicated reaction system beyond the one component yet, by design.
- ✅ DONE 2026-06-21 — Push-to-open doors: `DoorTransition` gained an opt-in **PUSH mode** (held lean-in for `push_seconds` ≈0.6s before it gives; a tap never opens it — deliberate friction / a dread tool). Reuses the door's existing target/spawn wiring; INTERACT/ENTER doors unchanged. Plays the toy-creak as a stand-in door SFX. Opted the **Hollow House ExitDoor** into it ("Shove it open"). `test_push_door.gd`. *(No dedicated door SFX asset yet — toy-creak stands in; swap the file, keep the key.)*
