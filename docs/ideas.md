...
- ~~**Sorceress.games asset bake-off (Phase 2)**~~ **DONE 2026-07-25** — scored [[decisions/2026-07-25-briar-pose-bakeoff]]: Arm A (Imagine) wins pose stand-ins and is wired interim (scale-matched to loco); PixelLab style-lock but pose-fail; Sorceress not run (human recipe in bakeoff folder). **Do not add Sorceress to stack yet.** WizardGenie still rejected.
...
- ~~**Bare fists deal NO damage to monsters.**~~ **DONE 2026-07-25** — unarmed melee skips `attack_hitbox.activate`; `Hurtbox` also blocks player→enemy without the gate. See `tests/test_flute_combat_gate.gd`.
- **Flute-gate monster interaction.** ✅ **SOOTHE half DONE 2026-06-21**. ✅ **COMBAT half DONE 2026-07-25**: pre-flute no player damage to monsters (melee/ranged/thrown); post-flute requires equipped weapon for armed attacks; thrown tools (group `thrown`) OK post-flute unarmed. `PlayerController.can_*` + `Hurtbox` gate. `test_flute_combat_gate.gd`.
- **Run-only before the flute.** Falls out of the two rules above — **code-gated 2026-07-25**; human F5 should confirm it *reads* as flee-only, not broken attack.
