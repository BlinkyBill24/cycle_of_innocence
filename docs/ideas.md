...
- **Add a runtime-visibility Godot MCP server** (from [[research/done/Claude-Godot-MCP-Servers-Research-2026-06-28]]): the installed `Coding-Solo/godot-mcp` v0.1.1 only runs the game + reads console errors — it **cannot see the live runtime tree or autoload state** (DreadManager, HollowingClock, VillageState, etc.) and carries an unpatched command-injection advisory (low risk, localhost/solo). Evaluate adding a runtime server so the AI can run the game and inspect the *running* tree/errors/stack traces. Preference order: **(1) `Erodenn/godot-mcp-runtime`** — injects its bridge at runtime and auto-cleans, so **nothing ships in web/Android export** (best fit for the hard Web constraint); **(2) `PrajnaAvidya/Godot-Peek-MCP`** — more debug power (live tree, stack traces, perf monitors) but ships an addon that must be export-excluded and is **Linux x86_64-only** (verify the binary runs on this Mint box first). **Avoid `IvanMurzak/Godot-MCP`** (needs C#/.NET → breaks Web export). Concrete payoff: settles the *wiring* half of the Briar "yellow blob" + locked-door "no feedback" diagnoses (did the behaviour/door logic actually fire?) — the *legibility* half stays a human-playtest question. Pairs with the GUT suite (deterministic pass/fail + observational runtime). When adopted: register via `claude mcp add`, point at the Godot binary, and add the export-exclude filter before the next web build (skip for `godot-mcp-runtime`). Not yet decided — adopting one is a real task with export implications.
