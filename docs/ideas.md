...
- **⚠ Verify LimboAI exports cleanly to Web — early** (from [[research/done/2026-06-20-ai-assisted-gamedev-orchestration-survey]]): LimboAI ships as a GDExtension (v1.6.0, gdextension-4.4) and GDExtension Web export has historically been fragile. Web is a locked hard constraint + LimboAI is a locked dependency, so a failed Web export would be a real problem. Do a throwaway Web export with a LimboAI behavior tree active and confirm it boots/runs in-browser *before* leaning harder on it. Risk to the locked stack.
