...
| Code agents | **Claude Code + Cursor** (existing) + **godot-mcp** (FOSS MCP server: run-game, **live runtime scene tree**, autoload inspection, run-GDScript-in-game, screenshots, console/error capture). **Adopted `Erodenn/godot-mcp-runtime` v3.1.2 on 2026-06-28** (`npx -y godot-mcp-runtime`, pinned), replacing the old `Coding-Solo/godot-mcp` base — chosen because it injects its bridge at runtime and leaves **nothing in the project → no web-export cleanup, nothing to ship-exclude** (best fit for the hard Web constraint), and it sidesteps the Coding-Solo command-injection advisory. Now the AI can see the *running* game's tree/autoload state (DreadManager/HollowingClock), not just scene files. See [[../research/done/Claude-Godot-MCP-Servers-Research-2026-06-28]]. | If more debug power is ever needed (stack traces, perf monitors, breakpoints): `PrajnaAvidya/Godot-Peek-MCP` — but it ships an addon you must export-exclude and is **Linux x86_64 only**. Avoid `IvanMurzak/Godot-MCP` (needs C#/.NET → breaks Web export). |
...
- godot-mcp (`Erodenn/godot-mcp-runtime` v3.1.2, **adopted 2026-06-28**) registered in `.mcp.json` via `npx -y godot-mcp-runtime@3.1.2` (env `GODOT_PATH=~/.local/bin/godot`). Addon-free — nothing lands in the project, so **no web/Android export cleanup needed**. Tools include `get_scene_tree`/`get_node_properties` (live runtime tree), `list_autoloads` (DreadManager/HollowingClock live state), `run_script` (GDScript in the running game), `run_project`/`get_debug_output` (boot-smoke + error capture), `take_screenshot`, `simulate_input`. ⚠️ **Loads only after a Claude Code restart** (project-scoped MCP servers prompt for approval on first load). Replaced the old `Coding-Solo/godot-mcp` base (no live tree + command-injection advisory). See [[../research/done/Claude-Godot-MCP-Servers-Research-2026-06-28]].
...
## Workflow rules
1. **Art**: Imagine prompt batches from [[art/imagine-prompts]] · **Pose stand-ins (2026-07-25):** Grok Imagine `image_edit` from V2 anchors beat PixelLab bitforge on *pose change* ([[decisions/2026-07-25-briar-pose-bakeoff]]); PixelLab stays for multi-dir grids. Sorceress not adopted pending human trial. → generate at 2–4× target size → downscale scripted or in GIMP/Pixelorama (interpolation: none; no Aseprite installed) → palette discipline pass → import with nearest filter. Always add human pixel-edit work on top of AI output (strengthens legal authorship position — no AI tool indemnifies). For *keeping concept art on-model* (anchor images, look-block templates, the no-seed caveat), see [[art/grok-reference-workflow]]. **Super Grok Heavy (confirmed 2026-06-21) does NOT change this pipeline** — it improves the concept/reference stage only; PixelLab stays canonical for sprites. `[verified 2026-06-21]`
