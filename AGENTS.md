...
- **R6 — Commit & push.** After meaningful changes: commit on the feature branch and push to `origin` — the **Forgejo** repo `home/cycle_of_innocence` (the single source of truth). Forgejo **auto-mirrors** every push to the private GitHub repo (off-box backup + what the claude.ai web Project reads). The user merges to `main` on the Forgejo web UI; never commit to `main` locally (R1). *(This repo was split from the `tchintchie/game` monorepo `test/` on 2026-06-17; see `docs/handbook.md` for the loop.)*
